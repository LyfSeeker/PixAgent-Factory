...
export async function POST(request: Request) {
  ...
  const task = text(body.task, 4_000);
  const skills = Array.isArray(body.skills) ? body.skills.filter((skill): skill is string => typeof skill === "string").slice(0, 8) : [];
  const tools = Array.isArray(body.tools) ? body.tools.filter((tool): tool is string => typeof tool === "string").slice(0, 8) : [];
