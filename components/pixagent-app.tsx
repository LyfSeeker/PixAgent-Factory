...
type Agent = { id: string; name: string; role: string; skills: string[]; tools: string[]; environment: "Local" | "Remote"; status: Status; task: string; progress: number; color: string; queue: number; spot: string };
type ScheduledTask = { id: string; agentId: string; title: string; dueAt: string; completed: boolean };

const seedAgents: Agent[] = [
  { id: "maya", name: "Maya Park", role: "Frontend Engineer", skills: ["React", "Next.js", "TypeScript"], tools: ["UI editor", "Browser", "Component tests"], environment: "Local", status: "working", task: "Build event dashboard", progress: 68, color: "#f18955", queue: 2, spot: "desk-a" },
  { id: "alex", name: "Alex Rivera", role: "Backend Engineer", skills: ["Node.js", "PostgreSQL", "APIs"], tools: ["Database", "API client", "Terminal"], environment: "Remote", status: "working", task: "Create events API", progress: 81, color: "#5ea8e5", queue: 1, spot: "desk-b" },
  { id: "leo", name: "Leo Bennett", role: "UI / UX Engineer", skills: ["Design systems", "Tailwind", "UX"], tools: ["Figma", "Visual checks", "Tokens"], environment: "Local", status: "complete", task: "Design component system", progress: 100, color: "#b987df", queue: 0, spot: "desk-c" },
  { id: "sara", name: "Sara Okafor", role: "QA Engineer", skills: ["Playwright", "Testing", "Debugging"], tools: ["Playwright", "Test runner", "Trace viewer"], environment: "Remote", status: "blocked", task: "Verify registration flow", progress: 32, color: "#efb557", queue: 2, spot: "desk-d" },
  { id: "max", name: "Max Chen", role: "Code Reviewer", skills: ["Architecture", "Security", "Review"], tools: ["Diff viewer", "Policy gates", "CI evidence"], environment: "Local", status: "approval", task: "Approve deployment plan", progress: 92, color: "#62c9a1", queue: 1, spot: "desk-e" },
];

const taskContracts = [
  { title: "Define event API contract", owner: "Alex Rivera", status: "in progress", blocker: "None", evidence: "OpenAPI schema + contract tests", decision: "Publish v1 contract", environment: "Remote", progress: 81 },
  { title: "Build event dashboard", owner: "Maya Park", status: "in progress", blocker: "Waiting for API schema v1", evidence: "Responsive page + visual test", decision: "Confirm empty-state copy", environment: "Local", progress: 68 },
  { title: "Verify registration flow", owner: "Sara Okafor", status: "blocked", blocker: "API returns 409 without error code", evidence: "Playwright trace + failing spec", decision: "Alex to classify conflict response", environment: "Remote", progress: 32 },
  { title: "Review deployment plan", owner: "Max Chen", status: "review", blocker: "Human approval required", evidence: "Security checklist + CI green", decision: "Approve staged deployment", environment: "Local", progress: 92 },
];

const nav = [
  { label: "Studio", href: "/workspace", icon: LayoutDashboard }, { label: "Agents", href: "/agents", icon: Users },
...
export function PixAgentApp() {
  ...
  const nextSchedule = useMemo(() => scheduledTasks.filter(task => !task.completed).sort((a, b) => a.dueAt.localeCompare(b.dueAt))[0], [scheduledTasks]);
  const navigate = (href: string) => router.push(href);
  const updateName = (name: string) => setAgents(old => old.map(a => a.id === selected.id ? { ...a, name } : a));
  const updateTask = (task: string) => setAgents(old => old.map(a => a.id === selected.id ? { ...a, task, status: task.trim() ? "working" : "idle" } : a));
  useEffect(() => { setNow(new Date()); const timer = window.setInterval(() => setNow(new Date()), 1000); return () => window.clearInterval(timer); }, []);
  useEffect(() => {
    if (!now) return;
    const due = scheduledTasks.filter(task => !task.completed && new Date(task.dueAt).getTime() <= now.getTime());
    if (!due.length) return;
    setScheduledTasks(old => old.map(task => due.some(item => item.id === task.id) ? { ...task, completed: true } : task));
    setAgents(old => old.map(agent => due.some(task => task.agentId === agent.id) ? { ...agent, status: "complete", progress: 100, task: due.find(task => task.agentId === agent.id)?.title ?? agent.task, queue: Math.max(0, agent.queue - 1) } : agent));
    const finished = due[0]; const owner = agents.find(agent => agent.id === finished.agentId);
    setNotice(`${owner?.name ?? "Coworker"} completed scheduled task: ${finished.title}`);
  }, [now, scheduledTasks, agents]);
  const scheduleTask = ({ title, agentId, dueAt }: Omit<ScheduledTask, "id" | "completed">) => {
    setScheduledTasks(old => [...old, { id: `schedule-${Date.now()}`, title, agentId, dueAt, completed: false }]);
    const agent = agents.find(item => item.id === agentId);
    setAgents(old => old.map(item => item.id === agentId ? { ...item, status: "waiting", task: title, queue: Math.max(1, item.queue) } : item));
    setNotice(`${agent?.name ?? "Coworker"} scheduled for ${new Date(dueAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`);
  };
  const runDemo = async () => { const worker = agents.find(a => a.id === "maya") ?? agents[0]; setNotice(`${worker.name} is working on a scoped task…`); try { const result = await fetch("/api/agents/run", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ taskId: "task-dashboard", agentId: worker.id, agentName: worker.name, role: worker.role, task: worker.task, skills: worker.skills, tools: worker.tools }) }); const payload = await result.json() as { output?: string; error?: string }; if (!result.ok) throw new Error(payload.error ?? "The task runner could not start."); setDemoStep(s => (s + 1) % 4); setAgents(old => old.map(a => a.id === worker.id ? { ...a, progress: Math.min(a.progress + 4, 99), status: "working" } : a)); setNotice(`${worker.name}: ${(payload.output ?? "Task response received.").replace(/\s+/g, " ").slice(0, 150)}`); } catch (error) { setNotice(error instanceof Error ? error.message : "The task runner could not start."); } };
  ...
  return <main className={`app-shell ${active === "Studio" ? "studio-mode" : ""}`}>
    ...
    <section className="workspace">
      ...
      {active === "Studio" ? <ReferenceStudio agents={agents} now={now} nextSchedule={nextSchedule} onSelect={setSelectedId} onHire={() => setShowHire(true)} onSchedule={() => setShowSchedule(true)} onDemo={runDemo} demoStep={demoStep} notice={notice} onNavigate={navigate}/> : <DashboardView title={active} agents={agents} metrics={metrics} onSelect={setSelectedId} onNavigate={navigate}/>}
    </section>
    {active === "Studio" && selectedId && <AgentDrawer agent={selected} onClose={() => setSelectedId("")} onName={updateName} onTask={updateTask} notice={notice}/>}
    {showHire && <HireModal onClose={() => setShowHire(false)} onHire={(agent) => { setAgents(a => [...a, agent]); setSelectedId(agent.id); setShowHire(false); }}/>} 
    {showSchedule && <ScheduleModal agents={agents} onClose={() => setShowSchedule(false)} onSchedule={(task) => { scheduleTask(task); setShowSchedule(false); }}/>}
  </main>;
}
...
function ReferenceStudio({ agents, now, nextSchedule, onSelect, onHire, onSchedule, onDemo, demoStep, notice, onNavigate }: { agents: Agent[]; now: Date; nextSchedule?: ScheduledTask; onSelect: (id: string) => void; onHire: () => void; onSchedule: () => void; onDemo: () => void; demoStep: number; notice: string; onNavigate: (url:string) => void }) {
  ...
  return <div className="reference-studio">
    ...
    <aside className="coworker-panel"><div className="panel-chrome"><Users size={19}/><span/><i/><i/><b>▣</b></div><div className="cream-panel"><p className="cream-eyebrow">YOUR COWORKERS</p><div className="roster-title"><h1>Everyone</h1><b>{agents.length}</b></div><div className="roster-tabs"><button className="selected">All</button><button>In office</button><button>Remote</button></div><div className="roster-list">{agents.map((agent, index) => <button className="roster-agent" key={agent.id} onClick={() => onSelect(agent.id)}><div className={`roster-avatar avatar-${index % 5}`}><span>{agent.name.split(" ").map(x=>x[0]).join("")}</span></div><div><b>{agent.name}</b><p>{agent.status === "idle" ? "Available for assignment" : agent.task}</p><small><i className={statusMeta[agent.status].dot}/> In office · {statusMeta[agent.status].label}</small></div><ChevronRight size={17}/></button>)}</div><div className="roster-actions"><button className="roster-hire" onClick={onHire}><Plus size={15}/> Add coworker</button><button className="roster-schedule" onClick={onSchedule}>◷ Schedule task</button></div></div><div className="pixel-notice">{notice}</div></aside>
  </div>;
}
...
function AgentDrawer({ agent, onClose, onName, onTask, notice }: {agent:Agent;onClose:()=>void;onName:(name:string)=>void;onTask:(task:string)=>void;notice:string}) {
  const [editingName, setEditingName] = useState(false); const [editingTask, setEditingTask] = useState(false);
  return <aside className="agent-drawer"><div className="drawer-top"><span><Users size={16}/> COWORKER PROFILE</span><button onClick={onClose}><X size={19}/></button></div><div className="agent-hero"><div className="portrait" style={{background:agent.color}}>{agent.name.split(" ").map(x=>x[0]).join("")}</div><div>{editingName ? <input autoFocus className="name-input" value={agent.name} onChange={event=>onName(event.target.value)} onBlur={()=>setEditingName(false)} onKeyDown={event=> event.key === "Enter" && setEditingName(false)}/> : <h2 onClick={()=>setEditingName(true)} title="Edit coworker name">{agent.name} <small>✎</small></h2>}<p>{agent.role}</p><span className={`status-pill ${statusMeta[agent.status].dot}`}><i/> {statusMeta[agent.status].label}</span></div></div><div className="drawer-section"><p className="section-label">CURRENT ASSIGNMENT</p><div className="assignment"><div>{editingTask ? <input autoFocus className="task-input" value={agent.task} onChange={event=>onTask(event.target.value)} onBlur={()=>setEditingTask(false)} onKeyDown={event=> event.key === "Enter" && setEditingTask(false)}/> : <button className="assignment-edit" onClick={()=>setEditingTask(true)} title="Assign or edit task"><b>{agent.task || "Assign a task"}</b><span>{agent.progress}% complete · click to edit</span></button>}</div><button className="assignment-arrow" onClick={()=>setEditingTask(true)} aria-label="Edit task"><ChevronRight size={18}/></button></div><div className="progress agent-progress"><i style={{width:`${agent.progress}%`}}/></div></div><div className="drawer-section"><p className="section-label">CAPACITY</p><div className="capacity"><div><span>Active work</span><b>{agent.queue} / 3 tasks</b></div><div className="capacity-bars"><i/><i className="filled"/><i className="filled"/></div></div></div><div className="drawer-section"><p className="section-label">SCOPED TOOLS · {agent.environment}</p><div className="skill-list">{agent.tools.map(tool=><span key={tool}>{tool}</span>)}</div></div><div className="drawer-section"><p className="section-label">SPECIALTIES</p><div className="skill-list">{agent.skills.map(s=><span key={s}>{s}</span>)}</div></div><div className="drawer-section"><p className="section-label">RECENT SIGNAL</p><div className="signal-note">{notice}</div></div><button className="drawer-cta"><ClipboardList size={17}/> View task history</button></aside>
}

function HireModal({onClose,onHire}:{onClose:()=>void;onHire:(agent:Agent)=>void}) { const [name,setName]=useState(""); const [role,setRole]=useState("Full-stack Engineer"); return <div className="modal-backdrop"><div className="hire-modal"><button className="modal-close" onClick={onClose}><X size={19}/></button><span className="modal-icon"><Bot size={23}/></span><p className="eyebrow">EXPAND YOUR CREW</p><h2>Hire an AI coworker</h2><p className="subtle">Give them a name and choose the specialization for your studio.</p><label>Agent name<input value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Nia Patel" autoFocus/></label><label>Primary role<select value={role} onChange={e=>setRole(e.target.value)}><option>Full-stack Engineer</option><option>Product Designer</option><option>DevOps Engineer</option><option>Data Engineer</option></select></label><button className="primary-btn wide" disabled={!name.trim()} onClick={()=>onHire({id:`agent-${Date.now()}`,name:name.trim(),role,skills:["TypeScript","Problem solving","Collaboration"],tools:["Scoped editor","Terminal","Test runner"],environment:"Local",status:"idle",task:"Available for assignment",progress:0,color:"#e9809c",queue:0,spot:"desk-e"})}>Hire {name || "agent"}</button></div></div> }

function ScheduleModal({ agents, onClose, onSchedule }: { agents: Agent[]; onClose: () => void; onSchedule: (task: Omit<ScheduledTask, "id" | "completed">) => void }) {
  const defaultTime = new Date(Date.now() + 5 * 60_000).toISOString().slice(0, 16);
  const [title, setTitle] = useState("Review scheduled build"); const [agentId, setAgentId] = useState(agents[0]?.id ?? ""); const [dueAt, setDueAt] = useState(defaultTime);
  return <div className="modal-backdrop"><div className="hire-modal schedule-modal"><button className="modal-close" onClick={onClose}><X size={19}/></button><span className="modal-icon">◷</span><p className="eyebrow">TIME-BOXED WORK</p><h2>Schedule a task</h2><p className="subtle">The office clock will complete this scoped task at the selected local time.</p><label>Task title<input value={title} onChange={event => setTitle(event.target.value)} placeholder="e.g. Review test evidence" autoFocus/></label><label>Task owner<select value={agentId} onChange={event => setAgentId(event.target.value)}>{agents.map(agent => <option key={agent.id} value={agent.id}>{agent.name} · {agent.role}</option>)}</select></label><label>Complete at<input type="datetime-local" value={dueAt} min={new Date().toISOString().slice(0, 16)} onChange={event => setDueAt(event.target.value)}/></label><button className="primary-btn wide" disabled={!title.trim() || !agentId || !dueAt} onClick={() => onSchedule({ title: title.trim(), agentId, dueAt: new Date(dueAt).toISOString() })}>Schedule task →</button></div></div>
}

function TaskBoard() { return <div className="task-contract-page"><div className="contract-heading"><p className="eyebrow">OBSERVABLE EXECUTION</p><h1>Scoped work queue</h1><p>Every task declares one owner, bounded tools, its current blocker, required evidence and the next human decision.</p></div><div className="contract-rules"><span>1 owner / task</span><span>Scoped tool capability</span><span>Evidence before hand-off</span><span>Human gates are explicit</span></div><div className="contract-list">{taskContracts.map(task => <article className={`contract-card ${task.status.replace(" ", "-")}`} key={task.title}><header><div><span className="contract-status">{task.status}</span><h2>{task.title}</h2></div><b>{task.progress}%</b></header><div className="contract-meta"><span><i>OWNER</i>{task.owner}</span><span><i>RUNNER</i>{task.environment}</span><span className={task.blocker === "None" ? "clear" : "blocked"}><i>BLOCKER</i>{task.blocker}</span></div><div className="contract-proof"><div><i>REQUIRED EVIDENCE</i><b>{task.evidence}</b></div><div><i>NEXT DECISION</i><b>{task.decision}</b></div></div><div className="contract-progress"><i style={{width:`${task.progress}%`}}/></div></article>)}</div></div> }

function DashboardView({title,agents,metrics,onSelect,onNavigate}:{title:string;agents:Agent[];metrics:{done:number;work:number;blockers:number};onSelect:(x:string)=>void;onNavigate:(x:string)=>void}) { if(title === "Agents") return <div className="dashboard-page"><div className="view-heading"><p className="eyebrow">YOUR AI WORKFORCE</p><h1>Agent command center</h1><p>Manage capacity, skills and execution state across your studio.</p></div><div className="agent-grid">{agents.map((a,index)=><button className="agent-card" key={a.id} onClick={()=>{onSelect(a.id);onNavigate("/workspace")}}><div className={`portrait sm grid-avatar avatar-${index % 5}`}>{a.name.split(" ").map(x=>x[0]).join("")}</div><span className={`status-pill ${statusMeta[a.status].dot}`}><i/> {statusMeta[ a.status].label}</span><h3>{a.name}</h3><p>{a.role}</p><div className="skill-list">{a.skills.slice(0,2).map(x=><span key={x}>{x}</span>)}</div><div className="mini-progress"><span>{a.task}</span><b>{a.progress}%</b><div className="progress"><i style={{width:`${a.progress}%`}}/></div></div></button>)}</div></div>;
  if(title === "Tasks") return <TaskBoard />;
