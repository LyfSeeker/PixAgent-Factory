  ...
  return <div className="reference-studio">
    <section className="pixel-office">
      ...
      <div className="office-room">
        ...
        {agents.map((agent, index) => <button className={`retro-desk ${agent.spot}`} key={agent.id} onClick={() => onSelect(agent.id)} aria-label={`Open ${agent.name}'s profile`}><div className="retro-monitor"><i/><span/></div><div className="retro-lamp"/><div className="retro-mug">☕</div><div className="retro-keyboard"/><span className={`worker-sprite worker-${index % 5}`}/>{agent.status === "blocked" && <b className="pixel-callout danger">!</b>}{agent.status === "approval" && <b className="pixel-callout approval">…</b>}{agent.status === "complete" && <b className="pixel-callout success">✓</b>}<span className="pixel-name">{agent.name.split(" ")[0]}<em className={statusMeta[agent.status].dot}/></span></button>)}
        <button className="cat-action" onClick={() => setCatMood(catMood === "Purring!" ? "Feed cat" : "Purring!")}><span>🐾</span>{catMood}</button><button className="walkabout" onClick={() => onNavigate("/projects")}><div className="walker"><i/><i/></div><b>You</b></button>
      </div>
